...
Let $V''=V \cup V'$, $\Gamma=\emph{Instantiate}(T_{\varphi}^{r, V''}, V')$ and $\Gamma_1=\emph{ERes}(\varphi,V)$, then %R(\NI(\Elm(\EXIST\FUTURE(\Gamma),\Gamma)))_{\CTL}$, then
\[
...
Planning via the model checking is based on generating plans by determining whether formula are true in model~\cite{giunchiglia1999planning}.
The fundamental ingredients include \emph{planning domain}, \emph{planning problem} and \emph{plan generation}.
...
\begin{definition}[Plan]\label{pro:plan}
 ...
 %We can see from~\cite{giunchiglia1999planning} that the problem of planning can be reduced to model checking $\EXIST\FUTURE \varphi$ with $\varphi$ being a propositional \CTL\ formula if we base our planning domain on a Kripke structure.
...
\end{enumerate}
 ...
 %The following model depicted in Figure~\ref{PD} from~\cite{akintunde2017planning} can be translated into a planning domain ${\cal D} =\tuple{{\cal F}, {\cal W}, \Lambda, \cal T}$ with
%\begin{enumerate}
%  \item ${\cal F} = \{p, q\}$,
%  \item ${\cal W} = \{\{p, q\}, \{p\}, \{q\}\}$, for convenience we use $s_0$ to represent $\{p, q\}$, $s_1$ to $\{p\}$ and $s_2$ to $\{q\}$,
%  \item $\Lambda = \{lock, unlock, load, unload, wait\}$,
%  \item ${\cal T}=\{(s_0, wait, s_0), (s_0, lock, s_1), (s_0, unlock, s_2)$, $(s_1, wait, s_1)$, $(s_1, load, s_2),(s_2, wait, s_2), (s_2, unload, s_1)\}$.
%\end{enumerate}
...
Besides, we can also use \CTL\ formula expressing the condition that gaol is reached within $n$ steps, which can be expressed in the following way:
\[
   \underbrace{\EXIST\NEXT(\EXIST\NEXT(\dots(goal)\dots))}_{n\ \EXIST\NEXT's}.
\]
The property ``Eventually reach the goal and permanently in a certain sate that satisfy some property $\varphi$ (a \CTL\ formula)" can be expressed in \CTL\ as:
\[
\EXIST \FUTURE(goal) \wedge \EXIST\GLOBAL \varphi.
\]
(For more information of using \CTL\ for goal specification, please see~\cite{akintunde2017planning}).
