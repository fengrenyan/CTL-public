...
For convenient, we use $V\subseteq \Ha$ denote the set we want to forget, $V' \subseteq \Ha$ with $V \cap V'={\O}$ the set of atoms ($I$ be the set of index) introduced in the transformation process, $\varphi$  the \CTL\ formula, $T_{\varphi}$ be the set of $\CTLsnf$ clause obtained from $\varphi$ by using transformation rules  and $\Hm=(S,R,L,[\_], s_0)$ unless explicitly stated.
 ...
 Let $T$ be a set of $\CTLsnf$ clauses, then $\NI(T)$, called \emph{nonInd}-$\CTLsnf$, is a set of formula obtained from $T$ by doing the following steps:
\begin{enumerate}[(i)]
  \item Replacing $P\supset \EXIST_{\tuple{ind}} \NEXT \varphi_1, \dots, P\supset \EXIST_{\tuple{ind}}\NEXT \varphi_n \in T$ with $P\supset \EXIST \NEXT \bigwedge_{i\in \{0,\dots, n\}}\varphi_i$,
  \item Replacing $P_1\supset \EXIST_{\tuple{ind}} \NEXT \varphi_1, \dots, P_n\supset \EXIST_{\tuple{ind}}\NEXT \varphi_n \in T$ with $\bigwedge_{e \in 2^{\{0,\dots, n\}} \setminus \{\emptyset\}}(\bigwedge_{i\in e}P_i\supset \EXIST \NEXT (\bigwedge_{i\in e}\varphi_i))$,
  \item Replacing $P\supset \EXIST_{\tuple{ind}} \FUTURE \varphi_1, \dots, P\supset \EXIST_{\tuple{ind}} \FUTURE \varphi_n \in T$ with $P\supset \bigvee\EXIST\FUTURE (\varphi_{j_1} \wedge \EXIST\FUTURE(\varphi_{j_2} \wedge \EXIST\FUTURE(\dots \wedge \EXIST\FUTURE \varphi_{j_n})))$, where $(j_1, \dots, j_n)$ are sequences of all elements in $\{0, \dots, n\}$.
\end{enumerate}
Where $P$, $P_i$ $(1\leq i\leq n)$ are conjunction of literals and $\varphi_i$ ($i\in \{0,\dots, n\}$) are any \CTL\ formulae.
We will not consider the case that there is $P_1\supset \EXIST_{\tuple{ind}} \FUTURE l_1, \dots, P_n\supset \EXIST_{\tuple{ind}}\FUTURE l_n \in T$ due to there is not the case in the process of our computing forgetting.
...
We denote this process as $\Sub(\Gamma, V')$.
...
Note that the conclusion of EF4 is not stated in normal form. Similarly with that in~\cite{zhang2009refined}, to present the
conclusion of EF4 in normal form, we introduce a set $V_F=\{w_1, w_2, w_3, w_4\}$ of new atomic propositions.
Then the conclusion of EF4 can be represented by the following set $W$ of $\CTLsnf$ clauses.
\begin{align*}
&\{\top \supset \neg P\vee C_3 \vee C_2 \vee w_1\} \\
%& w_1 \supset \EXIST_{\tuple{ind}} \NEXT(w_3 \vee C_2 \vee C_4)\} \\
%& \cup \{\top \supset \neg w_3 \vee \neg C_{3,i}| C_{3,i} \hbox{ is a literal in } C_3\}\\
& \cup \{w_1 \supset \EXIST_{\tuple{ind}} \NEXT(\neg C_{3,i} \vee C_2 \vee C_4 \vee w_2)| C_{3,i} \hbox{ is a literal in } C_3\}\\
& \cup \{w_2 \supset \ALL\NEXT w_3, w_3 \supset \ALL \FUTURE w_4, w_4 \supset C_3 \vee C_2\}
\end{align*}
Similarly, the conclusion of other rules (EF2 to EF4) can be represented by a set of $\CTLsnf$ clauses.
...
Let $V^F$ be a set of atom introduced by transforming the result of those rules into $\CTLsnf$. We have:
\begin{proposition} \label{pro:EF}
Let $\Gamma=T_{\varphi}^{r,V \cup V'}$, we have $\Gamma \equiv_{\tuple{V'\cup V^F, {\O}}}\EXIST\FUTURE(\Sub(\Gamma, V'))$.
\end{proposition}
\begin{proof}
...
We will show that $\forall (\Hm, s_0) \in \Mod(T_i)$ there is an initial Ind-structure $(\Hm',s_0')$ such that $(\Hm',s_0') \models T_{i+1}$ and $(\Hm',s_0') \lrto_{\tuple{V'\cup V^F, {\O}}} (\Hm, s_0)$
...
\label{alg:subn}
%\LinesNumbered %?????????
\KwIn{A set $\Gamma$ of $\CTLsnf$ clauses, a set of $\ALL$-step clauses, a set of $\EXIST$-step clauses and $V\subseteq \Ha$}% ????????
...
\begin{algorithm}[!h]
\caption{$\EXIST \NEXT(C, L)$}% ??????
\label{alg:subn}
%\LinesNumbered %?????????
\KwIn{A formula $C$}% ????????
\KwOut{A set $\CTLsnf$ clauses}% ????
$C == P \supset ((\neg C_3 \wedge \neg C_2) \supset  (\EXIST_{\tuple{ind}}\NEXT(C_3 \wedge \neg (C_2 \vee C_4) \supset
\ALL\NEXT \ALL\FUTURE(C_3 \vee C_2))))$\;
$L_1:=\{\top \supset  \neg P \vee C_3\vee C_2 \vee w_1\}$\;
$L_2 := \{w_1 \supset \EXIST_{\tuple{ind}}\NEXT (\neg l\vee C_2 \vee C_4 \vee w_2)| l \hbox{ is a literal in } C_3\}$\;
$L_3 := \{w_2 \supset \ALL\NEXT w_3, w_3 \supset \ALL \FUTURE w_4, w_4 \supset C_3 \vee C_2\}$\;
$L:= L_1 \cup L_2\cup L_3$\;
\Return $L$.
\end{algorithm}

\begin{algorithm}[!h]
\caption{$\ALL\NEXT(C, L)$}% ??????
\label{alg:subn}
%\LinesNumbered %?????????
\KwIn{A formula $C$}% ????????
\KwOut{A set $\CTLsnf$ clauses}% ????
...
Therefore, we can do the following elimination to eliminate them.
...
Let $V''=V \cup V'$, $\Gamma=\Sub(T_{\varphi}^{r, V''}, V')$ and $\Gamma_1=\Elm (\EXIST\FUTURE(\Gamma), \Gamma)$, then  $\Gamma_1\equiv_{\tuple{V\cup V' \cup V^F, {\O}}} T_{\varphi}^{r, V''}$ and $\Gamma_1 \equiv_{\tuple{V \cup V' \cup V^F, I}} \varphi$.
\end{proposition}
\begin{proof}
...
\end{enumerate}
Therefore, we have $\Gamma_1\equiv_{\tuple{V\cup V' \cup V^F, {\O}}} T_{\varphi}^{r, V''}$  by Proposition~\ref{pro:VI:div} and Proposition~\ref{pro:EF}.

And then $\Gamma_1 \equiv_{\tuple{V \cup V' \cup V^F, I}} \varphi$ follows.
\end{proof}
...
Remember that the auxiliary propositions have been introduced by the process of transforming a formula into a set of $\CTLsnf$ clauses is
only to rename those subformulae of $\varphi$~\cite{bolotov2000clausal}.
For convenience, let $\Omega$ be a sequence of sets of $\CTLsnf$ clauses, in which each set can be seen as a formula of conjunction of clauses in it, obtained from $\Elm (\EXIST\FUTURE(\Gamma), \Gamma)$ by using distribution law by viewing each $\CTLsnf$ as an atom.
Hence we have the following process.

Let $\Gamma=\Sub(T_{\varphi}^{r,V \cup V'}, V')$ and $\Gamma_1=\NI(\Omega)$, we can easily change $\Gamma_1$ into a formula of the form $\Omega_1= \bigwedge X \wedge \bigwedge_{p\in (V'\setminus \Gamma) \cup V^F} (p\supset \varphi_1\vee \dots \vee p \supset \varphi_n)$ with $\varphi$ is an instantiate formula of $\Gamma$ and $X$ is a set of formulas.
Then $R(\Gamma_1)$ is obtained from $\Omega_1$ by doing the following two steps for each $p\in (V'\setminus \Gamma) \cup V^F$:
\begin{itemize}
...
\begin{proposition}
Let $\Gamma=T_{\varphi}^{r,V \cup V'}$, $\Gamma_1=\Sub(\Gamma, V')$ and $\Gamma_2 =\Elm (\EXIST\FUTURE(\Gamma_1), \Gamma_1)$, then $\Gamma_2  \equiv_{\tuple{(V'\setminus \Gamma_1) \cup V^F, {\O}}} R(\NI(\Gamma_2))$. and $\varphi \equiv_{\tuple{V\cup V'\cup V^F,{I}}} R(\NI(\Gamma_2))_{CTL}$.
\end{proposition}
\begin{proof}
For each $p$ talked above is a name of the formula $\psi$, \ie $p \lrto \psi$.
Then $\Gamma_2  \equiv_{\tuple{(V'\setminus \Gamma_1) \cup V^F, {\O}}} R(\Gamma_2)$, and then $\Gamma_2 \equiv_{\tuple{V\cup V'\cup V^F, {\O}}} R(\Gamma_2)$  by (V) of Proposition~\ref{div}.

Therefore, $\varphi \equiv_{\tuple{V\cup V'\cup V^F,{I}}} \NI(R(\Gamma_2))_{CTL}$ by Proposition~\ref{pro:elm} and the definitions of $\NI$ and $T_{\CTL}$.
\end{proof}
