...
We are using binary bisimulation relation to relate \CTL\ and $\CTLsnf$, this show that how transform the \CTL\ formula into $\CTLsnf$ clauses and then return to \CTL\ formula after finish the computing process.
...
Although any \CTL\ formula can be transformed into a set of $\CTLsnf$ clauses, but it do introduce the \emph{index} and new atoms. Both the two problems are we should solved.
...
the paper Form 2000 to 2009, include LTL and CTL.
\subsection{Using Resolution Computing forgetting}
 in Propositional Logic, In Model Logic.
