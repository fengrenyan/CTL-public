...
The method is an extension of the resolution used to decide the satisfiability of \CTL\ formula.
...
We are using binary bisimulation relation to relate \CTL\ and $\CTLsnf$, this show that how to transform the \CTL\ formula into $\CTLsnf$ clauses and then return to \CTL\ formula after finishing the computing process.
Besides, we extend the original resolution rules by adding EF imply rules. This connects the \emph{next\ state} and \emph{future\ state}.
...
Although we have proposed an model-based to compute forgetting in \CTL\ in ``the theory paper", but both time and space complexity are 2-exponential. It is urgent to find an efficient algorithm.

For one thing, the existing algorithm of computing forgetting in different logics, including  propositional logic, answer set programming, description logic and modal logic, are not directly applicable in \CTL. For instance, in propositional forgetting theory, forgetting atom $q$ from $\varphi$ is equivalent to a formula $\varphi[q/\top] \vee \varphi[q/\perp]$, where $\varphi[q/X]$ is a formula obtained from $\varphi$ by replacing each $q$ with $X$ ($X\in \{\top, \perp\}$).
...
For another, as far as I know the existing methods to compute forgetting include the classical talked above and resolution-based approachs in propositional logic~\cite{lin1994forget,Yisong:2015:arx} and Ackermann-based approach (second-order elimination) in description logic~\cite{Zhao:2017:IJCAI}. However, the resolution and Ackermann-based methods need the normal form of the formula, it is hard to obtain such normal form in \CTL.
...
In this paper we extend the Resolution Calculus in~\cite{zhang2014resolution} to eliminate the atoms in the transformation process and combine the \CTL\ with $\CTLsnf$ by using the binary bisimulation relation (one is the set of atoms and another one is the set of indexes).
...
$T_{\varphi}, V' \lto \emph{Transform}(\varphi, V)$\;
...
In this case, if we do not specify $l$, $C_2$, $C_3$  and $C_4$ are instantiate formulae of  $\Sub(Res, V')$, it is easy check that all results including  $P\supset \EXIST_{\tuple{ind}}\NEXT (\neg l \vee C_2 \vee C_4)$ and $P\supset \ALL \NEXT (\neg l \vee C_2 \vee C_4)$ obtained from the \emph{Connect} process will be deleted in the Removing\_atoms process.
...
In their papers, the main idea is also to transform any \CTL\ formula into the normal form $\CTLsnf$.
But the \CTL\ is a kind of branch time temporal logic, they introduced the ``index" besides \start\ for that purpose.

All in all, a complete set of transform and resolution rules had been proposed for both PLTL and \CTL. And it shows that the transformation is satisfiability preserving and also for the result obtained from using the resolution rules on the normal form.
\subsection{Using Resolution Computing forgetting}
Resolution, a kind of methods of Second-order quantifier elimination, has been used to compute the forgetting in different logic~\cite{Yisong:2015:arx}. In those case, the formula is required to be a paradigm with a particular form.
...
Our method extend the resolution calculus in~\cite{zhang2014resolution} by adding removing those irrelevant atoms and transforming the result into \CTL\ formula. 
...
More important, our algorithm is correct and the time and space complexity of Algorithm~\ref{alg:compute:forgetting:by:Resolution} are $O((m+1)2^{4(n+n')}$.
