...
% activate next line to remove comments
\commentsfalse
...
\ifcomments
\newcommand{\yisongcomment}[1]{{\bf \textcolor{red} {*** YS: ***} #1 }} 
\newcommand{\comment}[1]{}
\else
\newcommand{\yisongcomment}[1]{} 
\newcommand{\comment}[1]{}
\fi

\ifcolors
\newcommand{\yisong}[2]{{}{{\textcolor{red}{#2}}}} 
\else
\newcommand{\yisong}[2]{#2} 
\fi
...
\begin{itemize}
  ...
  is fixed and finite. Thus, there are only finite number model structures. },
...
\end{theorem}
\begin{proof}
This can be proved by induct on the structures of $\phi$. % and supposing $\Var(\phi) \cap V = \Empty$ due to $\IR(\phi,V)$.
...
Let $\varphi_1= \neg p \wedge \ALL \NEXT q \wedge \EXIST \NEXT(x \supset \EXIST \NEXT x)$ and $\varphi_2 = q \wedge \ALL \NEXT q$ be two \CTL\ formulae.
 They are  $\{y\}$-irrelevant. One can check that $K_1$ and $K_2$ in Figure~\ref{yBisim} satisfy $\varphi_1$, and they do not satisfy $\varphi_2$.
\end{example}
...
\end{theorem}
\begin{proof}
(i) Let $k={ch({\cal M},V)}$, %we give the proof idea of it. 
One the one hand, one can verify that $({\cal M},s)\models{\cal F}_V(\Tr_n(s))$ holds for $n \geq 0$, which implies $(\Hm, s)\models {\cal F}_V(\Tr_k(s))$. 
...
Following from the knowledge forgetting point of view~\cite{Yan:AIJ:2009}, we show that the above forgetting repectes the four forgetting postulates: 
\begin{itemize}
...
\end{itemize}
where $\varphi,\varphi'$ are formuals and $V\subseteq\cal A$.
...
say that  forgetting results have no effect on formulas that are irrelevant to fogotten atoms; the postulate (\textbf{\IR}) states 
