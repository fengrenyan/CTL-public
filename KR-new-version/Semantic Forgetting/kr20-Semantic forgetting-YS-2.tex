...
\title{On Sufficient and Necessary Conditions in Bounded CTL}
...
\section{Preliminaries}
 ...
 %{\textcolor{blue}{Throughout this paper, we fix a finite set $\Ha$ of propositional variables (or atoms), and use $V$, $V'$ for subsets of $\Ha$.}}
Throughout this paper, we fix a finite set $\Ha$ of propositional variables (or atoms), use $V$, $V'$ for subsets of $\Ha$ and $\overline V = \Ha - V$.
...
\begin{itemize}
  ...
  is fixed and finite, \ie $S\subseteq\cal S$ with ${\cal S}=\{b_1,\ldots,b_m\}$,
  such that any \CTL\ formula with bounded length is satisfiable if and only if it is satisfiable in a such model structure. Thus, there are only finite number of model structures. },
  ...
  \item $L: S\rto 2^{\cal A}$ is a labeling function. %{\textcolor{blue}{$L$ is a labeling function $S\rto 2^{\cal A}$}}.
\end{itemize}
...
In the following we assume every formula of $\cal L$ has bounded size, where
the size $|\varphi|$ of formula $\varphi$ is its length over the alphbet of $\cal L$~\cite{DBLP:journals/jcss/EmersonH85}.
...
%%%Add for 毕业论文
{\textcolor{red} {The following facts are important for the Resolution-based Algorithm.}}
The \emph{Replacing\_atoms}($\Gamma$) is obtained from the following two steps:
\begin{enumerate}[(i)]
  \item If there is a set of clauses $C$ satisfy the post-condition of the Trans($i$) with $1\leq i \leq 12$, then let $\Gamma = (\Gamma \setminus C) \cup \{\alpha\}$ with the $\alpha$ is the pre-condition of Trans($i$), else go to (ii).
  \item for each $p\rto \psi$ with $p\in V'$, replacing $p$ with $\psi$.
\end{enumerate}

 \begin{theorem}
 Let $\psi$ be a \CTL\ formula, then \emph{Replacing\_atoms}$(\Gamma) \equiv_{\tuple{V, I}} \psi$.
 \end{theorem}
 \begin{proof}
 We prove the Trans(11), other rules can be prove similarly.

 The ($i$) above is obvious by Proposition 3. We will show ($ii$) from the following two points:

 (A) $p$ do not instantiated by some instantiate formula. This can be proved by induction on the
structures of $\varphi$ (Without loss of generality, we assume $p\rto \varphi$ is a $\CTLsnf$ clause since any formula can be translated into a set of $\CTLsnf$ clauses).

case 1: $\varphi = a$ is a proposition formula, then $a \in V$. It is easy check that $q \rto \textbf{P} (\varphi \UNTIL l) \equiv_{\tuple{\{a\}, I}} q \rto \textbf{P} \FUTURE l$.

case 2: $\varphi = \EXIST \NEXT C_1$ and $a \in \Var(C_1) \cap C$. In this case we have $q \rto \textbf{P} (\varphi \UNTIL l) \models q \rto \textbf{P} \FUTURE l$. For each model $(\Hm, s_0)$ of $q \rto \textbf{P} \FUTURE l$ we can construct a model $(\Hm', s_0)$ the same as $(\Hm, s_0)$ except for each state $s \in \pi_{s_0}$ (note $\textbf{P} \in \{\EXIST, \ALL\}$) before the sate $s'$ (may be $s=s'$) with $(\Hm, s') \models l$ we add or delete $a$ for $L(s)$. It is easy check that $(\Hm', s_0)\models q \rto \textbf{P} (\varphi \UNTIL l)$ and $(\Hm, s_0) \equiv_{\tuple{\{a\}, I}} (\Hm', s_0)$. Therefore, $q \rto \textbf{P} (\varphi \UNTIL l) \equiv_{\tuple{\{a\}, I}} q \rto \textbf{P} \FUTURE l$.

Other cases can be proved similarly.

(B) $a\in \Var(l) \subseteq V$. We prove it from the following two aspects.

(a) $p$ do not instantiated by some instantiate formula. In this case, all the clauses include $p$ were deleted before, i.e. $\Gamma = \Gamma \setminus C$ ($C$ is replaced with $\top$). Therefore, for any initial \Ind-structure $(\Hm, s_0)$ we can construct an initial \Ind-structure $(\Hm', s_0)$ the same as $(\Hm, s_0)$ except $L(s_0) \models a$ (if $l$ is a positive literal) or $L(s_0) \not\models a$ (else).
It is easy check $(\Hm', s_0)\models q \rto \textbf{P} (\varphi \UNTIL l)$ and $(\Hm, s_0) \equiv_{\tuple{\{a\}, I}} (\Hm', s_0)$. Therefore, $q \rto \textbf{P} (\varphi \UNTIL l) \equiv_{\tuple{\{a\}, I}} q \rto \textbf{P} \FUTURE l$.

(b) $p$ is instantiated by some instantiate formula. In this case $p\rto \varphi$ is changed into $\varphi\rto \varphi \equiv \top$ by (ii) above. Hence, we can construct the model as (a).
 \end{proof}

%\begin{align*}
% \EXIST(\varphi \UNTIL \psi) & \equiv  \EXIST \FUTURE \psi \wedge (\psi \vee (\varphi \wedge \EXIST \NEXT(\varphi \vee \psi))) \wedge\\
%& \ALL\GLOBAL((\varphi \wedge \EXIST \NEXT(\varphi \vee \psi)) \rto \EXIST \NEXT(\psi \vee (\varphi \wedge \EXIST \NEXT(\varphi \vee \psi)))).
%\end{align*}
%\begin{proof}
%($\Rto$) For all model $(\Hm, s_0) \in \Mod(\EXIST(\varphi \UNTIL \psi))$. By the semantics of $\EXIST \UNTIL$ there is a path $\pi_{s_0}$ s.t. for some state $s_i$ with $i \geq 0$, $(\Hm, s_i) \models \psi$, and then $(\Hm, s_0) \models \EXIST \FUTURE \psi$. Besides, $(\Hm, s_j) \models \varphi$ for each $0\leq j < i$.
%\end{proof}
...
%{\textcolor{red}{In this section we introduce the notion of set-based bisimulation.
%Intuitively, bisimulation relates states that mutually mimic all individual transitions~\cite{Baier:PMC:2008}, while the Set-based bisimulation relates states that mutually mimic all individual transitions on a set of atoms, this is close with forgetting some set of atoms.}}

In the following we present the notion of bisimulation on a given signature. It is (somehow) an extension of the classical bisimulation for \CTL\ in \cite{Baier:PMC:2008}.
...
On the one hand, the above set-based bisimulation is an extension of the
bisimulation-equivalence of Definition~7.1 in \cite{Baier:PMC:2008} in the
sense that if $V=\cal A$ then our bisimulation is almost the same as the
latter.
%\footnote{The latter has a given set of initial states,
%while there is only one initial state in our case.}.
On the other hand, the above set-based bisimulation notion is similar to
...
The $(iv)$ in this proposition shows that if a \MPK-structure is $V_1$ and $V_2$-bisimilar with the other two \MPK-structures respectively, then those two \MPK-structures are $V_1 \cup V_2$-bisimilar. This is important for forgetting.
%since it lays the foundation of forgetting atoms in $V$ one by one.
Besides, the $(v)$ means that if two \MPK-structures are  $V_1$-bisimilar then they are $V_2$-bisimilar for each $V_2$ with $V_1\subseteq V_2 \subseteq \Ha$.
...
\end{itemize}
%Note that
The last two conditions in the above definition
...
\end{proposition}
\begin{proof}(sketch)
($\Rto$) $\Tr_j(s_1)\lrto_V\Tr_j(s_2)$ for every $0\le j\le n$ is apparent since $(s_1,s_2)\in{\cal B}_n$ implies that $(s_1,s_2)\in{\cal B}_j$ for every $0 \le j \le n$.
...
As a basis, we first give the definition of characterizing formulas of computation trees.
...
Besides, given a set of atomic propositions $V$,  it is easy to check that any initial \MPK-structure has its own characterizing formula on $V$. We will see later that the characterizing formula is critical to prove some important properties of forgetting and  to compute the SNC and WSC of a property under an initial \MPK-structure.
...
%{\textcolor{red}{Given a set of atomic propositions $V$, two kripke structures are bisimilar w.r.t. $V$'s complement iff one of them models the others characterizing formula. Moreover, the $V$-bisimulation between two initial \MPK-structures implies the semantic equivalence of the corresponding characterizing formulas. Formally:}}

\begin{theorem}\label{CF}
Let $V\subseteq \Ha$, $\Hm=(S,R,L,s_0)$ and $\Hm'=(S',R', L',s_0')$ be two model structures. Then,
\begin{enumerate}[(i)]
...
On the other hand, $({\cal M},s)\models{\cal F}_V(\Tr_n(s'))$ implies
 ...
 (i) ($\Lto$) This is apparent due to $(\Hm, s_0) \models {\cal F}_V(\Hm, s_0)$ and $\IR({\cal F}_V(\Hm, s_0), \overline V)$, hence $(\Hm',s_0') \models {\cal F}_V(M,s_0)$ by Theorem~\ref{thm:V-bisimulation:EQ}.
...
% {\textcolor{red}{Given a formula $\psi=\varphi\wedge (q\lrto \alpha)$, if $\IR(\varphi \wedge \alpha, \{q\})$, then the result of forgetting $q$ from $\psi$ is $\varphi$. (I don't think I explained it well.)}}
 Given a formula $\varphi\wedge (q\lrto \alpha)$ with $q \not \in \Var(\varphi) \cup \Var(\alpha)$. Let us show what this implies for forgetting.
...
The following proposition shows that forgetting a set of atoms can be obtained by forgetting atoms in the set one by one.
...
We give the proof of (i), others can be proved similarly.
 ...
 For one thing, for all model ${\cal K}=(\Hm, s_0)$ of the left side there is a model ${\cal K}'=(\Hm', s_0')$ of $\ALL\NEXT\phi$ such that ${\cal K} \lrto_V {\cal K'}$, i.e. for all $s_1$ with $(s_0,s_1) \in R$ there exists $s_1'$ with $(s_0',s_1')\in R'$ such that $(\Hm, s_1) \lrto_V (\Hm', s_1')$ and then $(\Hm,s_1) \models  \CTLforget(\phi,V)$ due to $\IR(\CTLforget(\phi,V), V)$ and $(\Hm', s_1') \models \phi$ and versa vice. Therefore, ${\cal K} \models \ALL\NEXT \CTLforget(\phi,V)$.
...
$(\Hm, s_0) \leftrightarrow_V (\Hm',s_0')$ and $(\Hm',s_0')\nvDash \psi$. So checking whether $\psi$ is not irrelevant to $V$ can be achieved in the following steps: (1) guess two initial \MPK-structures $(\Hm,s_0)$ and $(\Hm',s_0')$ such that $(\Hm,s_0) \models \psi$ and $(\Hm',s_0')\nvDash \psi$, and (2) check
...
Let $\psi = sl \rto (\ALL\NEXT sr \vee \EXIST \NEXT le \vee \EXIST \NEXT(se \vee lc))$, $\varphi = sl \rto \ALL\NEXT sr$, $\Ha =\{sl, sr, se, lc, le\}$ and $V = \{sl, sr\}$, then we can check  that the WSC of $\psi$ on $V$ under the initial  \MPK-structure $(\Hm, s_0)$ in Figure~\ref{BVM} is $\varphi$.

We verify this result by the following two steps:
\begin{enumerate}[(i)]
  \item It is apparent that $\varphi \models \psi$ and $\Var(\varphi) \subseteq V$. Besides, $(\Hm, s_0) \models \varphi \wedge \psi$, hence $(\Hm, s_0) \models \varphi \rto \psi$, which means $\varphi$ is a SC of $\psi$ on $V$ under $(\Hm, s_0)$,
  \item We will show that for any SC $\varphi'$ of $\psi$ on $V$ under $(\Hm, s_0)$, there is $(\Hm, s_0) \models \varphi' \rto \varphi$. Following Figure~\ref{carVB}, we can see that $(\Hm', s_0) \lrto_{\Ha \setminus V} (\Hm, s_0)$. By Theorem~\ref{CF} we can easily obtain the characterizing formula of $(\Hm, s_0)$ on $V$, \ie ${\cal F}_V(\Hm, s_0)\equiv {\cal F}_V(\Hm', s_0) = sl \wedge \neg sr \wedge \ALL\GLOBAL ((sl \wedge \neg sr) \rto \ALL \NEXT (\neg sl \wedge sr)) \wedge \ALL \GLOBAL((\neg sl \wedge sr) \rto \ALL\NEXT (sl \wedge \neg sr))$, due to $ch(\Hm', \Ha \setminus V) =0$.
...
\begin{figure}[ht]
  ...
  \caption{A model structure $\Hm'$ with an initial state $s_0$}\label{carVB}
\end{figure}
...
%\begin{definition}\label{formulaNS}
   ...
   % \end{definition}
    It turns out that the previous notions of SNC and WSC for an atomic proposition can be lifted to any formula, or conversely the SNC and WSC of any formula can be reduced to that of a proposition.
...
%    \\
    ...
    We know that $\varphi \wedge q \models \psi$ iff ${\cal F} \models \psi$ by {\bfseries (PP)}, due to $\emph{IR}(\psi, (\Var(\varphi) \cup \{q\})- V)$. Hence, $\varphi \wedge {\cal F} \models \psi$ by $\varphi \wedge q \models \psi$ (by suppose).
...
\begin{proposition}\label{pro:time:alg1}
 Let $\varphi$ be a CTL formula and $V\subseteq \Ha$ with $|{\cal S}|=m$, $|\Ha|=n$ and $|V|=x$. Then the space complexity is $O((n-x)m^{2(m+1)}2^{nm})$ and the time complexity of Algorithm~\ref{alg:compute:forgetting:by:VB} is at least the same as the space.%and the space complexity is $O(2^{2m})$, where $|\Ha| = m$.
\end{proposition}
\begin{proof}
Supposing each state or atom occupy one byte, then a state pair $(s, s')$ occupy two bytes.
For any $B\subseteq {\cal S}$ with $B \not = \emptyset$ and $s_0\in B$, we can construct an initial \MPK-structure $(\Hm, s_0)$ with $\Hm=(B, R, L, s_0)$, in which there is at most $\frac{|B|^2}{2}$ state pairs in $R$ and $|B|*n$ $(s, A)$ ($A \subseteq \Ha$) in $L$. Hence, the $(\Hm, s_0)$ occupy at most $|B|+|B|^2 + |B|*n$ bytes. Besides, there is at most $|B|^{|B|+1}*2^{nm}$ number of initial \MPK-structures.
% and then for $B$ there is at most $|B|*(|B|+\frac{|B|^2}{2} + |2^{\Ha}|^{|B|}) = |B|^2+\frac{|B|^3}{2} + |B|*(2^n)^{|B|}$ space should be needed.
% Therefore, for ${\cal S}$ there is at most $Ps$ space to store those initial \MPK-structure with
% \begin{align*}
% & Ps= (1^2+\frac{1^3}{2}+1*(2^n)^1)+(2^2+\frac{2^3}{2}+2*(2^n)^2)\\
% & + \dots + (m^2+\frac{m^3}{2}+m*(2^n)^m)\\
% & = \frac{m(m+1)(2m+1)}{6} + \frac{m^2(m+1)^2}{2} + \\
% & \frac{(2^n)^{m+1}(2^n*m-m-1) + 2^n}{(2^n-1)^2}
% \end{align*}
Therefore, there is at most $m^{m+2}*2^{nm}$ number of initial \MPK-structures, hence it will at most cost $m^{m+2}*2^{nm}*(m+m^2+nm)$ bytes.
...
Moreover, a further study regarding the computational complexity for other general fragments is required and part of the future research agenda. Such investigation can be coupled with fine-grained parameterized analysis.
 ...
 \noindent\textbf{Proposition}\ref{pro:time:alg1} Let $\varphi$ be a CTL formula and $V\subseteq \Ha$ with $|{\cal S}|=m$, $|\Ha|=n$ and $|V|=x$. The the space complexity is $O((n-x)m^{2(m+1)}2^{nm})$ and the time complexity of Algorithm~\ref{alg:compute:forgetting:by:VB} is at least the same as the space.%and the space complexity is $O(2^{2m})$, where $|\Ha| = m$.
 \begin{proof}
 Supposing each state or atom occupy one byte, then a state pair $(s, s')$ occupy two bytes.
 For any $B\subseteq {\cal S}$ with $B \not = \emptyset$ and $s_0\in B$, we can construct an initial \MPK-structure $(\Hm, s_0)$ with $\Hm=(B, R, L, s_0)$, in which there is at most $\frac{|B|^2}{2}$ state pairs in $R$ and $|B|*n$ $(s, A)$ ($A \subseteq \Ha$) in $L$. Hence, the $(\Hm, s_0)$ occupy at most $|B|+|B|^2 + |B|*n$ bytes. Besides, there is at most $|B|^{|B|+1}*2^{nm}$ number of initial \MPK-structures.
 % and then for $B$ there is at most $|B|*(|B|+\frac{|B|^2}{2} + |2^{\Ha}|^{|B|}) = |B|^2+\frac{|B|^3}{2} + |B|*(2^n)^{|B|}$ space should be needed.
 ...
 Let $k=n-x$, for any initial \MPK-structure ${\cal K}=(\Hm, s_0)$ with $i\geq 1$ nodes, in the worst, i.e., $ch(\Hm,V)=i$, we will spend $N(i)$ space to store the characterizing formula.
 \begin{align*}
  & N(i)=(k+(\dots + (k + 2ik)*(2i))\dots *(2i))\\
  & = (2i)^0k + 2ik + (2i)^2k+ \dots + (2i)^{(i-1)}k\\
  & = \frac{(2i)^i -1}{2i-1} k.
