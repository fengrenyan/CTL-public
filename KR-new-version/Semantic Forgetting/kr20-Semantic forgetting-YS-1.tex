...
\begin{proposition}\label{pro:time:alg1}
 Let $\varphi$ be a CTL formula and $V\subseteq \Ha$ with $|{\cal S}|=m$, $|\Ha|=n$ and $|V|=x$. The the space complexity is $O((n-x)*(2*m^3)^{m^3}+ 2^{(n*(m+1))})$ and the time complexity of Algorithm~\ref{alg:compute:forgetting:by:VB} is at least the same as the space.%and the space complexity is $O(2^{2m})$, where $|\Ha| = m$.
\end{proposition}
\begin{proof}
For any $B\subseteq {\cal S}$ with $B \not = \emptyset$ and $s_0\in B$, we can construct an initial \MPK-structure $(\Hm, s_0)$ with $\Hm=(B, R, L, s_0)$, in which there is at most $\frac{|B|^2}{2}$ relations in $R$ and $|2^{\Ha}|^{|B|}$ label functions $L$. Hence, the $(\Hm, s_0)$ occupy at most $|B|+\frac{|A|^2}{2} + |2^{\Ha}|^{|B|}$ space and then for $B$ there is at most $|B|*(|B|+\frac{|B|^2}{2} + |2^{\Ha}|^{|B|}) = |B|^2+\frac{|B|^3}{2} + |B|*(2^n)^{|B|}$ space should be needed.
...
\begin{align*}
 ...
 & = \frac{(2i)^i -1}{2i-1} k.
\end{align*}
In the worst case, i.e., there is $i=\frac{m(m+1)(2m+1)}{6}$ initial \MPK-structure with $m$ nodes, we will spent $i*N(i)$ space to store the result of forgetting.
...
Therefore, the space complexity is $O((n-x)*(2*m^3)^{m^3}+ 2^{(n*(m+1))})$ and the time complexity is at least the same as the space.
\end{proof}
