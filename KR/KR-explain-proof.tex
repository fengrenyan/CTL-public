...
\begin{example}
Let $K_1$, $K_2$ be two initial \MPK-structures described as Figure~\ref{yBisim}, it easy check $K_1 \lrto_{\{y\}} K_2$.
\begin{figure}[ht]
  \centering
  % Requires \usepackage{graphicx}
  \includegraphics[width=8cm]{yBisim.png}\\
  \caption{Two $\{y\}$-bisimulation initial \MPK-structures}\label{yBisim}
\end{figure}
\end{example}
...
\end{theorem}
\begin{proof}
This theorem can be proved by inducting on the formula $\phi$ and supposing $\Var(\phi) \cap V = \O$ due to $\IR(\phi,V)$.

For the case $\phi = \psi_1 \vee \psi_2$, the induction hypothesis is ${\cal K}_1 \models \psi_i$ iff ${\cal K}_2 \models \psi_i$ with $i\in \{1,2\}$. Then we can see that ${\cal K}_1 \models \phi$ iff ${\cal K}_1 \models \psi_1$ or ${\cal K}_1 \models \psi_2$ iff ${\cal K}_2 \models \psi_1$ or ${\cal K}_2 \models \psi_2$ by induction hypothesis.

Other cases can be proved similarly.
\end{proof}

\begin{example}
Let $\varphi_1= \neg p \wedge \ALL \NEXT q \wedge \EXIST \NEXT(x \supset \EXIST \NEXT x)$ and $\varphi_2 = q \wedge \ALL \NEXT q$ be two \CTL\ formulae that irrelevant with $\{y\}$. We can check that both $K_1$ and $K_2$ in Figure~\ref{yBisim} satisfy $\varphi_1$ and do not satisfy $\varphi_2$.
\end{example}
...
\end{proposition}
\begin{proof} 
From left to the right is apparent since $(s_1,s_2)\in{\cal B}_n$ implies that $(s_1,s_2)\in{\cal B}_m$ for every $0 \le m \le n$.

For the other direction, in order to show $(s_1,s_2)\in{\cal B}_n$ we need only to prove for any $s_1'$ with $(s_1, s_1') \in R_1$ there is $s_2'$ with $(s_2, s_2')\in R_2$ \st $(s_2, s_2') \in {\cal B}_{n-1}$ and vice versa.
$\Tr_0(s_1)\lrto_V\Tr_0(s_2)$ implies $(s_1, s_2) \in B_0$, $\Tr_1(s_1)\lrto_V\Tr_1(s_2)$ implies for any $s_1'$ with $(s_1, s_1') \in R_1$ there is $s_2'$ with $(s_2, s_2')\in R_2$ \st $(s_2, s_2') \in B_0$ and vice versa, hence $(s_1, s_2) \in B_1$. Therefore, we can prove $(s_1,s_2)\in{\cal B}_n$ recursively.
\end{proof}
...
\end{theorem}
\begin{proof}
(i) Let $k={ch({\cal M},V)}$, we give the proof idea of it. For one thing, we can check $({\cal M},s)\models{\cal F}_V(\Tr_n(s))$ for $n \geq 0$ and then $(\Hm, s)\models {\cal F}_V(\Tr_k(s))$. For another, $({\cal M},s)\models{\cal F}_V(\Tr_n(s'))$ implies 
  $\Tr_n(s) \lrto_{\overline V} \Tr_n(s')$. In this case, we can easily proof that for each $s'\in S$, $({\cal M},s) \lrto_{\overline V} ({\cal M},s')$ if and only if $({\cal M},s')\models{\cal F}_V(\Tr_k(s))$. And then obtained our result.
  
(ii) This is obtained from Lemma~\ref{lem:Vb:TrFormula:Equ}.
\end{proof}
...
Let $\psi = sl \supset (\ALL\NEXT sr \vee \EXIST \NEXT le \vee \EXIST \NEXT(se \vee lc))$, $\varphi = sl \supset \ALL\NEXT sr$, $\Ha =\{sl, sr, se, lc, le\}$ and $V = \{sl, sr\}$, then we can check as follows that the WSC of $\psi$ on $V$ under our initial  \MPK-structure $(\Hm, s_0)$ in Figure~\ref{BVM} is $\varphi$.

We check this result from the following two steps:
\begin{enumerate}[(i)]
