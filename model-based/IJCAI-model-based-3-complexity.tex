...
Weakest precondition, we also call weakest sufficient condition (WSC), is introduced by Dijkstra in~\cite{dijkstra1978guarded}.
Strongest postcondition (we also call strongest necessary condition (SNC)), a dual concept, was introduced subsequently.
...
However, in model checking of concurrent system with there is at least one successor state for each state $s$ in this system, the termination of this system is impossible.
...
Let $\varphi=\ALL\GLOBAL \ALL \FUTURE(p\wedge r)$, which means $p\wedge r$ will be satisfied infinite times in the structure, be a \CTL\ formula with $r$ expressing orange juice.
\end{example}
...
To present a formal definition of forgetting, we need the concepts of $V$-bisimulation.
...
Let $\Hm,s_0$ be an I-structure, $\varphi$ be a \CTL\ formula and $V$ a set of atoms. Deciding whether $\Hm,s_0$ is a model of $\forget(\varphi, V )$ is NP-complete.
\end{proposition}
%\begin{proof}
...
Let $\varphi$ and $\psi$ be two CTL($\ALL \FUTURE$) (a fragment of \CTL, in which each formula contains only $\ALL \FUTURE$ temporal connective) formulas and $V$ a set of atoms. Then we have the
results:
\begin{enumerate}[(i)]
  \item deciding whether $\forget(\varphi, V ) \models \psi$ is co-NP-complete,
  \item deciding whether $\psi \models \forget(\varphi, V)$ is $\Pi_2^P$-complete,
  \item deciding whether $\forget(\varphi, V) \models \forget(\psi, V)$ is $\Pi_2^P$-complete.
\end{enumerate}
...
The theorem implies:
\begin{corollary}
Let $\varphi$ and $\psi$ be two CTL($\ALL \FUTURE$) formulas and $V$ a set of atoms. Then
\begin{enumerate}[(i)]
  ...
  \item deciding whether $\forget(\varphi, V) \equiv \forget(\psi, V)$ is $\Pi_2^P$-complete.
\end{enumerate}
\end{corollary}
...
As we have said the complexity of Algorithm~\ref{alg:compute:forgetting:by:VB} is $O(2^{2^m})$ (very inefficient), a future
...
%
%(iv) Let ${\cal M}_i=(S_i,R_i,L_i)~(i=1,2,3)$, $s_1 \lrto_{V_1} s_2$ via a binary relation $\Hb$, and $s_2 \lrto_{V_2} s_3$ via a binary relation $\Hb''$. Let $\Hb' \subseteq S_1 \times S_3$ and $\Hb' = \{(w_1, w_3)| (w_1, w_2)\in \Hb$ and $(w_2, w_3)\in \Hb_2\}$. It's apparent that $(s_1, s_3) \in \Hb'$. We prove $\Hb'$ is a $V_1 \cup V_2$-bisimulation between $s_1$ and $s_3$ from the three points of Proposition~\ref{Vbi:Equ} of $X$-bisimulation (where $X$ is a set of atoms). For all $(w_1, w_3) \in \Hb'$:
%\begin{enumerate}[(a)]
...
%\end{enumerate}
%
%(v) We will show that $({\cal K}_1, {\cal K}_2) \in \Hb_n$ for all $n \ge 0$ inductively.
%
...
%    \begin{proof}
%	Let $\varphi' =\varphi \cup\{q\lrto\alpha\}$. For any model $({\cal M},s)$ of $\CTLforget(\Gamma', q)$ there is an initial \MPK-structure $({\cal M}',s')$ \st\ $({\cal M},s)\lrto_{\{q\}}({\cal M}',s')$ and $({\cal M}',s') \models \varphi'$. It's apparent that $({\cal M}',s') \models \varphi$, and then $({\cal M},s) \models \varphi$ since $\IR(\varphi,\{q\})$ and $({\cal M},s)\lrto_{\{q\}}({\cal M}',s')$
...
%
%
%\textbf{Proposition}\ref{modelChecking}.
%\begin{proof}
%The problem can be determined by the following two things: (1) guessing
%an I-structure $\Hm',s_0'$ satisfying $\varphi$; and
%(2) checking if  $(\Hm, s_0) \leftrightarrow_V (\Hm', s_0')$. Both two steps can be done in polynomial time.
