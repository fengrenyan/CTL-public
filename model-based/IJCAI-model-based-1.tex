...
In model checking, specification is mainly used to specify the properties that should be satisfied by a system. %~\cite{chaki2005concurrent,clarke1994model,ghosh2016arsenal}.
...
%In this case, we have two aspects to solve this problem:
%\begin{enumerate}[(i)]
%  \item If we only interest the $p$-relate part of $\varphi$, then we should forgetting the atom $r$ from $\varphi$;
%  \item We can find the WSC of $\varphi$ on $V=\{p,r\}$ under $(\Hm,s_0)$.
%\end{enumerate}
\end{example}
...
%Therefore, we think about forgetting the atoms that are not in $V$ in order to get the conditions that we need, and then it becomes important to study the forgetting of CTLS.
...
%However, the method to compute the necessary and sufficient conditions under the give transition system $T$, set of atoms $V$ and a formula (or property) $\varphi$, \ie a formula $\psi$ on $V$ such that $T\models \psi\supset \varphi$ and $T\models \varphi \supset \psi$ respectively, is still lacking.
...
%In general, it is difficult to obtain such conditions for satisfying some properties under the given transition system.
%Inspired by the forgetting-based method to compute SNC (WSC)~\cite{DBLP:journals/ai/Lin01},  in this paper, we explore the forgetting of a branching-time temporal logic-\CTL\ (Computation Tree temporal Logic) to compute SNC and WSC.
